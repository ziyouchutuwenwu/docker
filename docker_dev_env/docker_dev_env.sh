...
docker run --rm -d --network=isolated_network --name apache-php -p 8888:80 -v ~/projects/docker/web_root:/app -e XDEBUG_REMOTE_AUTOSTART=1 -e XDEBUG_REMOTE_ENABLE=1 webdevops/php-apache-dev:alpine
...
docker run --rm -d --network=isolated_network --name redis-server -p 6379:6379 redis
